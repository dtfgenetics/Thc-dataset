      ...
      <div className="issue-list">
        {filtered.map((item) => {
          ...
          <article key={item.id} className="issue-row">
            <div className="issue-thumb"><ResilientImage sources={first ? referenceMediaSources(first, item.slug) : []} alt={first?.alt ?? `Reference image for ${item.name}`} fallback={<ImagePlaceholder label={`No usable licensed reference image available for ${item.name}`} />} /></div>
