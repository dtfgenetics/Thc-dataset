  ...
  return (
    <div className="view-container library-view">
      ...
      <div className="issue-list">
        {filtered.map((item) => {
          const displayMedia = resolvedDisplayMediaForIssue(item, issues)
          const first = displayMedia[0]?.media
          return (
          <article key={item.id} className="issue-row">
            <div className="issue-thumb"><ResilientImage sources={[first?.thumbnailUrl, first?.url]} alt={first?.alt ?? `Reference image for ${item.name}`} fallback={<ImagePlaceholder label={`No usable licensed reference image available for ${item.name}`} />} /></div>
            ...
            <div className="issue-row-status"><span className={`review review-${item.reviewStatus}`}>{item.reviewStatus}</span><span>{displayMedia.length ? `${displayMedia.length} reviewed visual reference${displayMedia.length === 1 ? '' : 's'}` : <><ImageOff size={15} /> Image gap</>}</span><span>{item.sources.length} source{item.sources.length === 1 ? '' : 's'}</span><button onClick={() => openIssue(item)} aria-label={`Open ${item.name}`}>Open guide <ArrowRight size={17} /></button></div>
          </article>
          )
        })}
      </div>
