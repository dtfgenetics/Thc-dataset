...
export function CoverageDashboard() {
  ...
  return (
    <div className="view-container coverage-view">
      ...
      <section className="coverage-table-wrap"><div className="coverage-table-heading"><div><h2>Image collection plan</h2><p>Counts below include only media with display permission and approved reference/training review status.</p></div><span>{currentImages} / {totalTarget}</span></div><div className="coverage-table" role="table" aria-label="Dataset image coverage by category"><div className="coverage-row header" role="row"><span>Category</span><span>Records</span><span>Approved images</span><span>Initial target</span><span>Coverage</span></div>{categoryOrder.map((category) => { const categoryIssues = issues.filter((issue) => issue.category === category); const imageCount = categoryIssues.flatMap((issue) => issue.media).filter(isDisplayableMedia).length; const target = targetByCategory[category] ?? 0; const percent = target ? Math.min(100, Math.round((imageCount / target) * 100)) : 0; return <div className="coverage-row" role="row" key={category}><strong>{category}</strong><span>{categoryIssues.length}</span><span>{imageCount}</span><span>{target}</span><div className="coverage-progress"><i style={{ width: `${percent}%` }} /><small>{percent}%</small></div></div>})}</div></section>
