...
const rootZoneRelevantCategories = new Set([
  'Nutrient deficiency',
  'Nutrient toxicity',
  'Root pathogen',
  'Water / root-zone',
...
export function rankDifferentials(
  ...
  return records
    .map((issue) => {
      ...
      if (rootZoneRelevantCategories.has(issue.category) && !context.ph) missing.push('measured pH')
      if (rootZoneRelevantCategories.has(issue.category) && !context.ec) missing.push('measured EC/PPM')
      ...
      // These categories can be ranked as plausible differentials, but visual evidence
      // alone cannot justify a high-confidence diagnosis.
