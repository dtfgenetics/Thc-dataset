...
def make_rag(profile: dict) -> tuple[list[dict], list[dict]]:
    ...
    for source in profile.get("sources") or []:
        ...
        if not (source.get("doi") or source.get("url")) or not claims:
            quarantine.append({"profile_id":profile.get("id"),"reason":"source_missing_provenance_or_supported_claims","source":source})
            continue
        for idx, claim in enumerate(claims):
            ...
            good.append({
                ...
                "source_id": sid,
                "source": {
                    "title": source.get("title"), "organization": source.get("organization"),
                    "publisher": source.get("publisher"), "authors": source.get("authors") or [],
                    "publicationDate": source.get("publicationDate"), "year": source.get("year"),
                    "accessedDate": source.get("accessedDate"), "doi": source.get("doi"), "url": source.get("url")
                },
...
def evidence_block(rows: list[dict], limit: int = 5) -> str:
    ...
    for row in rows[:limit]:
        parts.append(f"[{row['source_id']}] {row['claim']}")
    return "\n".join(parts)
...
def make_sft(profile: dict, rag_rows: list[dict]) -> list[dict]:
    ...
    diagnostic={
        ...
        "messages":[
            {"role":"system","content":"Answer only from the supplied evidence. Separate observation, differential diagnosis, and confirmation. Cite source identifiers and state uncertainty."},
    ...
    education={
        ...
        "messages":[
            {"role":"system","content":"Teach clearly from supplied evidence. Distinguish established facts from limitations and avoid adding uncited facts."},
...
def build(input_path: pathlib.Path, eval_path: pathlib.Path) -> tuple[list[dict],list[dict],list[dict],dict]:
    ...
    for profile in load_jsonl(input_path):
        ...
        if profile.get("reviewStatus") != "reviewed":
            quarantine.append({"profile_id":pid,"reason":"profile_not_reviewed","reviewStatus":profile.get("reviewStatus")})
            continue
        profile_rag, source_quarantine = make_rag(profile)
        quarantine.extend(source_quarantine)
        rag.extend(profile_rag)
        candidates=make_sft(profile, profile_rag)
        for item in candidates:
            ...
            if sha(norm(user_text)) in eval_fps:
                ...
                sft.append(item)
    # global exact-claim dedup: retain first occurrence but preserve all source ids in manifest statistics
    deduped=[]; seen_claims=set(); duplicate_claims=0
    for row in rag:
        fp=row["claim_sha256"]
        if fp in seen_claims:
            duplicate_claims += 1; continue
        seen_claims.add(fp); deduped.append(row)
    rag=deduped
    stats={
        "input_profiles":len(seen_profiles),"duplicate_profile_ids":len(duplicate_profiles),
        "rag_claims":len(rag),"sft_examples":len(sft),"quarantine_records":len(quarantine),
        ...
        "policy":"reviewed profiles only; source-level claim provenance required; context-required SFT; exact claim dedup; eval prompt collision rejection"
    }
...
def main() -> int:
    ...
    args=ap.parse_args()
    try: rag,sft,quarantine,stats=build(args.input,args.eval)
    except (OSError,ValueError) as exc:
        print(str(exc),file=sys.stderr); return 1
    if stats["rag_claims"] == 0 or stats["sft_examples"] == 0:
        print("corpus build produced no usable RAG/SFT records",file=sys.stderr); return 1
    if not args.check_only:
        ...
        (args.out/"manifest_v1.json").write_text(json.dumps(stats,indent=2,sort_keys=True)+"\n",encoding="utf-8")
